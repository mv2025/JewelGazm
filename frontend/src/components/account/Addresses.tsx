    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {mockAddresses.map((address) => (
          <div key={address.id} className="border border-border/60 rounded-xl p-6 bg-white hover:border-gold/30 transition-colors shadow-sm relative group">
            {address.isDefault && (
            ...
            <div className="flex items-start gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-xs font-sans text-gray-500 uppercase tracking-widest">{address.type} Address</p>
              </div>
            </div>

            <div className="space-y-1 mb-6 text-sm text-gray-600 font-sans">
              <p>{address.street}</p>
              <p>{address.city}, {address.state} {address.zip}</p>
              <p>{address.country}</p>
              <p className="pt-2 text-gray-800">Phone: {address.phone}</p>
            </div>
